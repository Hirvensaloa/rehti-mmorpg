...
bool RehtiGraphics::addCharacterObject(int characterID, std::vector<CharacterVertex> vertices, std::vector<uint32_t> indices, ImageData texture, std::array<Animation, ANIMATION_TYPE_COUNT> animations, std::vector<BoneNode> bones, std::vector<glm::mat4> transformations, glm::vec3 location, float rotation)
{
    ...
    for (size_t i = 0; i < bones.size(); i++)
    {
        ...
        charData.boneTransformations[i] = glm::mat4(1.f); // identity
    }
...
void RehtiGraphics::createObjectManager()
{
    ...
    pObjectManagerM = std::make_shared<GraphicsObjectManager>(instanceM, gpuM, logDeviceM, graphicsQueueM, indices.graphicsFamily.value(), kConcurrentFramesM);
...
void RehtiGraphics::createGraphicsPipeline()
{
    ...
    for (ObjectType objectType : getObjectTypes())
    {
        ...
        pipelinelayoutInfo.setLayoutCount = pObjectManagerM->getLayoutCount(objectType);
